...
class Affine_Plotter:
    ...
    def set_labels(self,ax,errors,offset,width,ylabel):
        ...
        ax.set_xticklabels(list(np.take(errors,[0],axis=1)), size=7, rotation=45)
    ...
    def new_reg_plot(self,destination,x,y,xlabel = '', ylabel = '',int='on'):
        ...
        if int=='on':
            ...
            ax.text(0.02,0.635,tstring3,transform=ax.transAxes,verticalalignment='top',fontsize='small')
            if not xlabel == '':
                ax.set_xlabel(xlabel)
            if not ylabel == '':
                ax.set_ylabel(ylabel)
    ...
    def get_plty(self,x,y,custom_x=None):
        ...
        return sy,ty,ay,res2[0][0],res1[0][0]
    ...
    def multi_reg_plot(self,destination,data,xlabel='',ylabel='',labels=[],lines = False):
        ...
        co = self.ref_reg(fit,expt,fit,expt)[0]
        #plty = np.dot(fit,co.T)
        stuff = self.get_plty(full_mat[:,0],full_mat[:,1])
        ax.plot(full_mat[:,0],stuff[2],'-k')
    ...
    def bivariate_plot2(self,destination,data1,data2,data3,leg_labels,xlabel='',ylabel='',pt_labels=[], origin = False,rms=[]):
        ...
        if pt_labels:
            for label,x,y in zip(pt_labels,datax,datay):
                ax.annotate(label,xy=(x,y),textcoords='offset points',xytext=(-15,8),fontsize=7.5)
        if xlabel:
            ax.set_xlabel(xlabel)
        if ylabel:
            ax.set_ylabel(ylabel)
    ...
    def ref_BS(self,x,y,limit=200,seed=23):
        ...
        #x_ = np.reshape(np.take(x,[1],axis=1),(-1,))
        #tm,tb,errs,nerrs,plty,mae,nmae,mse,nmse,rms,nrms,std,nstd, tmae,tmse,trms,tstd = self.regression(x_,y) 
        #base_arms = nrms
        #base_trms = trms
        #mae,mse,rms,std = self.ref_reg(x,y)
        #base_rrms = rms
        ...
        for i in range(limit):
            ...
            #x_ = np.reshape(np.take(rand_x,[1],axis=1),(-1,))        
            #m,b,errs,nerrs,plty,mae,nmae,mse,nmse,rms,nrms,std,nstd, tmae,tmse,trms,tstd = self.regression(x_,rand_y)
            #bsarms = nrms
            #bstrms = trms
            #bsmse = mse
            ...
            #bootstrap
            #x_ = np.reshape(np.take(test_x,[1],axis=1),(-1,))
            #plty = np.reshape(np.poly1d((m,b))(x_),(-1,))
            #aerrors = plty-test_y
            #terrors =x_ -bsmse-test_y
            #amae,amse,arms,astd = self.get_errors(aerrors)
            ...
            #print test_x
            #print bsrrms, rrms
            err_mat[i][0] = np.exp(-1)* bsrrms + (1-np.exp(-1)) * rrms
            err_mat[i][1] = rrms
    ...
    def ref_short_combo(self,mat1,mat2,index=0,key ='632BS',mod=1.0):
        nvar = mat2.shape[1] + 2
        ...
        for i,cou in enumerate(lcount):
            ...
            if key =='TENCV':
                base_epe = self.TENCV(x_,y,num=5)
            if key =='LOOCV':
                base_epe = self.LOOCV(x_,y)
            succeed = True
            min_epe = base_epe
            while succeed:
                min_epe,umat,vmat,succeed = self.check_for_best(ox,y,umat,vmat,min_epe,key,mod)
            umat.sort()
            vmat.sort()
    ...
    def mod_ref_short_combo(self,mat,x,index=0,key ='632BS'):
        """mat1 is a blah
        mat2 is a blah2
        """
        nvar = mat.shape[1] -1
        ...
        vmat = range(nvar)
        umat = []
        y=np.reshape(np.take(mat,[index],axis=1),(-1,))
        lcount = [1]
        #lcount.remove(index)
        #res = np.zeros((mat1.shape[1]-1,6))
        #coeffs = np.zeros((mat1.shape[1]-1,nvar))
        ...
        for i,cou in enumerate(lcount):
            ...
            count = 1
            while succeed:
                min_epe,umat,vmat,min_v,succeed = self.mod_check_for_best(ox,y,umat,vmat,min_epe,key)
                x_ = np.take(ox,umat,axis=1)
                co,mae,mse,rms,std = self.ref_reg(x_,y,x_,y)
    ...
    def mod_check_for_best(self,x,y,umat,vmat,min,key):
        ...
        if len(vmat) == 0:
            succ = False
        return min,umat,vmat,min_v,succ
    ...
    def nvar_graph(self,destination,res,names):
        gnames =['null']
        ...
        ax2 = ax1.twiny()
        ax1.plot(np.arange(res.shape[0]),np.take(res,[1],axis=1), '+',markersize=10.5, label='rms')
        ax1.plot(np.arange(res.shape[0]),np.take(res,[2],axis=1),'x', markersize=10.5, label = 'epe')
        ax1.set_xlabel('nvar')
    ...
    def make_epe_mat(self,model_list,ref_mat,target_data,key='632BS'):
        #target_data needs to be in x-y format
        epe_mat = np.zeros((len(model_list),ref_mat.shape[1]+1))
        for i,m in enumerate(model_list):
            co = self.ref_reg(m,target_data,m,target_data)[0]
            new_x_errs = np.reshape(np.dot(m,co),(-1,1))-target_data
            mat = np.hstack((np.zeros((ref_mat.shape[0],1)),ref_mat))
            epe_mat[i] = self.mod_ref_short_combo(mat,new_x_errs,key=key)[0][:,-1]

        return epe_mat

    #Make a heat plot
    def make_heat_plot(self,destination,data,type='rms',xticks=None,yticks=None,ylabel=None,xlabel=None,subset=[],tick_labels = [],minor_labels=True, norm_index = None):
        ...
        if minor_labels:
            if not subset:
                if not tick_labels and not type=='epe2':
    ...
    def full_run(self, filename, parent='.', limit=1000):
        ...
        for i in range(1):
            ...
            self.error_bar_plot(destination + '/' + 'Error_plot_rms_for_vs_' + headings[i], np.take(error_mat,[0,7,9,8],axis=1),legend_labels = ['Standard','Translation Corrected', 'Affine Corrected'], ylabel = 'rms', yerr=np.reshape(np.take(error_mat,[8],axis=1),(rel_rmss.shape[0],))*rel_rmss/100.)
        ...
        if int:
            for i in range(1):
                ...
                for j in range(len(headings))[i+1:]:
                    ...
                    np.savez(destination+'/'+gname,x=y, y=x, new_y=plty, slope=m, intercept=b, MAE=mae,MAE_fit=nmae, MSE=mse, MSE_fit=nmse, RMS=rms, RMS_fit=nrms, STD = std, STD_fit=nstd)
                np.savez(destination+'/'+'Error_file_for_vs_' + headings[i],Errors = error_mat)
                self.error_bar_plot(destination + '/' + 'Error_plot_rms_for_vs_' + headings[i] +'.png', np.take(error_mat,[0,9,12,11,10],axis=1),legend_labels = ['Standard','Slope Corrected','Translation Corrected', 'Affine Corrected'], ylabel = 'rms')
            ...
            for i in range(1):
                ...
                np.savez(destination+'/'+'Error_file_for_vs_' + headings[i],Errors = error_mat)
                self.error_bar_plot(destination + '/' + 'Error_plot_rms_for_vs_' + headings[i] +'.png', np.take(error_mat,[0,7,9,8],axis=1),legend_labels = ['Standard','Translation Corrected', 'Slope Corrected'], ylabel = 'rms')
